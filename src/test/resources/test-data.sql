...
SELECT create_elabel('dassco','MEDIA_CREATED_BY');      --# Media -> Pipeline
...
select * from cypher('dassco', $$
   ...
   MERGE (m)-[MC:MEDIA_CREATED_BY  {created_date: '2023-03-01 15:23:08'}]->(p)
   MERGE (a)-[:OWNED_BY]->(co)
   MERGE (a)-[:PROPERTY_OF]->(i)
   MERGE (a)-[:PART_OF_COLLECTION]->(c)
   MERGE (a)-[:HAS_MEDIA]->(m)
   MERGE (a)-[MDC:METADATA_CREATED_BY {created_date: '2023-03-01 15:23:08'}]->(d)
...
select * from cypher('dassco', $$
   ...
   MERGE (w)-[:STATIONED_AT]->(i)
   MERGE (m)-[MC:MEDIA_CREATED_BY {created_date: '2023-02-10 07:00:57'}]->(p)
   MERGE (a)-[:OWNED_BY]->(co)
   MERGE (a)-[:PROPERTY_OF]->(i)
   MERGE (a)-[:PART_OF_COLLECTION]->(c)
   MERGE (a)-[:HAS_MEDIA]->(m)
   MERGE (a)-[MDC:METADATA_CREATED_BY {created_date: '2023-02-10 07:00:57'}]->(d)
   MERGE (m)-[:EQUIPTMENT_USED]->(e)
   MERGE (m)-[:CREATED_ON]->(w)
   RETURN i
$$) as (institute agtype);
...
select * from cypher('dassco', $$
   ...
   MERGE (w)-[:STATIONED_AT]->(i)
   MERGE (m)-[MC:MEDIA_CREATED_BY {created_date: '2023-03-01 15:51:01'}]->(p)
   MERGE (a)-[:OWNED_BY]->(co)
   MERGE (a)-[:PROPERTY_OF]->(i)
   MERGE (a)-[:PART_OF_COLLECTION]->(c)
   MERGE (a)-[:HAS_MEDIA]->(m)
   MERGE (a)-[MDC:METADATA_CREATED_BY {created_date: '2023-03-01 15:51:01'}]->(d)
   MERGE (m)-[:EQUIPTMENT_USED]->(e)
   MERGE (m)-[:CREATED_ON]->(w)
   RETURN i
$$) as (institute agtype);
...
select * from cypher('dassco', $$
   ...
   MERGE (w)-[:STATIONED_AT]->(i)
   MERGE (m)-[MC:MEDIA_CREATED_BY {created_date: '2023-02-28 15:51:01'}]->(p)
   MERGE (a)-[:OWNED_BY]->(co)
   MERGE (a)-[:PROPERTY_OF]->(i)
   MERGE (a)-[:PART_OF_COLLECTION]->(c)
   MERGE (a)-[:HAS_MEDIA]->(m)
   MERGE (a)-[MDC:METADATA_CREATED_BY {created_date: '2023-02-28 15:51:01'}]->(d)
   MERGE (m)-[:EQUIPTMENT_USED]->(e)
   MERGE (m)-[:CREATED_ON]->(w)
   RETURN i
$$) as (institute agtype);
...
select * from cypher('dassco', $$
   ...
   MERGE (w)-[:STATIONED_AT]->(i)
   MERGE (m)-[MC:MEDIA_CREATED_BY {created_date: '2023-02-27 15:51:01'}]->(p)
   MERGE (a)-[:OWNED_BY]->(co)
   MERGE (a)-[:PROPERTY_OF]->(i)
   MERGE (a)-[:PART_OF_COLLECTION]->(c)
   MERGE (a)-[:HAS_MEDIA]->(m)
   MERGE (a)-[MDC:METADATA_CREATED_BY {created_date: '2023-02-27 15:51:01'}]->(d)
   MERGE (m)-[:EQUIPTMENT_USED]->(e)
   MERGE (m)-[:CREATED_ON]->(w)
   RETURN i
$$) as (institute agtype);
...
select * from cypher('dassco', $$
    MERGE (i:Institute { name: "NNAD", ocr_text: "FLORA DANICA EXSICCATA Lycopodium selago L. Jyll Silkeborg Vesterskov YII 1904 leg. M. Lorenzen.", geographic_region: "", taxon_name: "" })
   MERGE (d:Digitiser { name: "Margaret MDCclurkin" })
   MERGE (co:CopyrightOwner { name: "NNAD" })
   MERGE (p:Pipelines {name: "PL_AC002" })
...
select * from cypher('dassco', $$
   ...
   MERGE (w)-[:STATIONED_AT]->(i)
   MERGE (m)-[MC:MEDIA_CREATED_BY {created_date: '2023-02-25 15:51:01'}]->(p)
   MERGE (a)-[:OWNED_BY]->(co)
   MERGE (a)-[:PROPERTY_OF]->(i)
   MERGE (a)-[:PART_OF_COLLECTION]->(c)
   MERGE (a)-[:HAS_MEDIA]->(m)
   MERGE (a)-[MDC:METADATA_CREATED_BY {created_date: '2023-02-25 15:51:01'}]->(d)
   MERGE (m)-[:EQUIPTMENT_USED]->(e)
   MERGE (m)-[:CREATED_ON]->(w)
   RETURN i
$$) as (institute agtype);
