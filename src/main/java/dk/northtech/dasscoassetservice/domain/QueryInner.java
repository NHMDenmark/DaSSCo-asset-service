...
public class QueryInner {
    ...
    public Map<String, Map<String, Object>> toBasicPostgreSQLQueryString(String column, String table, int index) {
        String eventFilter = table.equals("event")
                && !Objects.equals(column, QueryItemField.CHANGE_LIST.getDisplayName())
                        ? "(#BASE# and %s)".formatted(this.eventTypeByMetadataColumn(column))
                        : "#BASE#";
        ...
        if (column.equalsIgnoreCase("currently_audited") && operator.equalsIgnoreCase("equal")) {
            ...
            String sql = """
                    %s (SELECT 1 FROM (
                        ...
                        AND e.event IN ('AUDIT_ASSET', 'UPDATE_ASSET', 'UPDATE_ASSET_METADATA', 'BULK_UPDATE_ASSET_METADATA')
                        ORDER BY e.timestamp DESC LIMIT 1
                    ) latest WHERE latest.event = 'AUDIT_ASSET')"""
        ...
        if (operator.equalsIgnoreCase("in")) {
            ...
            String sql = eventFilter.replace(
                    "#BASE#",
                    "(:%s = ANY(%s))".formatted(preparedParam, column));
        ...
        if (operator.equalsIgnoreCase("between")) {
            ...
            String preparedParamSecond = "%s2_%s".formatted("end_date", index);

            String[] times = value.split("#");

            if (times.length == 2) {

                long first = Long.parseLong(times[0]) / 1000;
                long second = Long.parseLong(times[1]) / 1000;

                String sql = eventFilter.replace(
                        "#BASE#",
                        "%s %s to_timestamp(:%s) and to_timestamp(:%s)".formatted(
                                column,
                                operator,
                                preparedParamFirst,
                                preparedParamSecond));
                ...
                return Map.of(
                        sql,
                        Map.of(preparedParamFirst, first, preparedParamSecond, second));
            }
        }
        if (table.equals("event")) {
            return Map.of(eventFilter.replace("#BASE# and ", ""), Map.of());
        }
        ...
        if (dataType.equals(DATE)) {
            return switch (operator.toLowerCase()) {
                case "between" -> {
                    ...
                    yield match + property + " >= " + dates[0] + " and " + match + property + " <= " + dates[1]; // if
                                                                                                                 // between,
                                                                                                                 // the
                                                                                                                 // value
                                                                                                                 // is
    ...
    public String eventTypeByMetadataColumn(String column) {
        switch (column) {
            case "asset_created_by":
                return " event = 'CREATE_ASSET'";
            case "asset_deleted_by", "date_asset_deleted_ars":
                return " event = 'DELETE_ASSET'";
            case "asset_updated_by", "date_asset_updated_ars":
                return " event = 'UPDATE_ASSET'";
            case "audited":
                return " event %s 'AUDIT_ASSET'".formatted(value.equalsIgnoreCase("true") ? "=" : "!=");
            case "audited_by":
                return " event = 'AUDIT_ASSET'";
            case "date_asset_created_ars":
                return "event = 'CREATE_ASSET'";
            case "date_audited":
                return " event = 'AUDIT_ASSET_METADATA'";
            case "date_metadata_created_ars":
                return " event = 'CREATE_ASSET_METADATA'";
            case "date_metadata_updated_ars":
                return " event = 'UPDATE_ASSET_METADATA'";
            case "date_pushed_to_specify":
                return " event = 'SYNCHRONISE_SPECIFY'";
            case "metadata_created_by":
                return " event = 'CREATE_ASSET_METADATA'";
